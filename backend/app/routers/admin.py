...
from app.schemas import LoginAttemptOut, SecurityStats
...
def stats(db: Session = Depends(get_db), days: int = Query(default=14, le=90)):
    ...
    total = len(rows)
    success = sum(1 for r in rows if r.status.value in ("mfa_success",))
    locked = sum(1 for r in rows if r.status.value in ("locked", "rate_limited"))
